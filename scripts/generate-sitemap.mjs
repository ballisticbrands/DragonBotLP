 ...
 * src/data/lpPages.js or src/data/competitors.js. This script derives the URL
 * list from the SAME source scripts/prerender.mjs uses — every directory under
 * dist/ that holds an index.html — so the sitemap and the prerender cannot
 * drift apart. Adding a route to lpPages.js is enough; nothing here changes.
 *
 ...
 *  • app / auth / admin surfaces — the product lives on app.getdragonbot.com,
 *    but keep the guard so a future in-repo route can't leak in.
 *  • /v1 /v2 /v3 — retained design iterations of the homepage. Same content as
 *    "/", so submitting them asks Google to index duplicates of our own LP. */
const EXCLUDE = [
